...
def send_and_receive_dm(slack_bot_client: WebClient, slack_user_client: WebClient, message: str, timeout_secs: int = 180 ) -> Any:
    ...
    original_msg_ts = SlackManager.add_message_to_channel(slack_user_client, channel=channel, message=message)
    logger.info(f"Posted message to channel {channel}")

    # Wait for the bot to respond
    reply = SlackManager.poll_for_reply(slack_user_client, channel=channel, original_message_ts=original_msg_ts, timeout_seconds=timeout_secs)
    logger.info(f"Received message from bot: {reply}")

    return reply

def send_dm_with_optional_timeout(slack_bot_client: WebClient, slack_user_client: WebClient, message_text: str, expected_text: str = None):

    if expected_text is None:
        return send_and_receive_dm(slack_bot_client, slack_user_client, message_text, timeout_secs=20)
    else:
        return send_and_receive_dm(slack_bot_client, slack_user_client, message_text)
    
def send_channel_msg_with_optional_timeout(slack_bot_client: WebClient, slack_user_client: WebClient, message_text: str, channel: Dict[str, Any], tag_bot: bool=False, expected_text: str = None):
...
def send_and_receive_channel_message(slack_user_client: WebClient, slack_bot_client: WebClient, message: str, channel: Dict[str, Any], tag_bot:bool=False, timeout_secs: int = 180) -> Any:
...
def update_channel_config(bot_id: str, user_performing_action: DATestUser, channel_name: str | None = None, updated_config_data: Dict[str, Any] = {}) -> Dict[str, Any]:
    ...
    for channel_config in channel_configs:
        ...
        if channel_name is None and channel_config["is_default"] == True:
...
def assert_button_presence(blocks: Dict[str, Any], action_id: str, should_exist: bool, test_name: str, channel_name: str="DM"):
    found_button = False
    for block in blocks:
        if block["type"] == "actions":
            for element in block.get("elements", []):
                if element.get("type") == "button" and element.get("action_id") == action_id:
                    found_button = True
                    break
            if found_button:
                break
    if should_exist:
        ...
        assert not found_button, f"{test_name}: Button should NOT be present in {channel_name}"


def extract_dm_slack_context(slack_test_context: Dict[str, Any]) -> Tuple[WebClient, WebClient, DATestUser, str]:
    return (
...
def extract_channel_slack_context(slack_test_context: Dict[str, Any]) -> Tuple[WebClient, WebClient, DATestUser, str, Dict[str, Any], Dict[str, Any]]:
    return (
        slack_test_context["slack_bot_client"],
        slack_test_context["slack_user_client"],
        slack_test_context["admin_user"],
