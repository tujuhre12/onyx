...
whereas 'use the knowledge graph (or KG) to summarize...' should be a knowledge graph query.
 ...
 preapre the plan for multiple search queries over multiple generation with the goal of having \
 each search query be quite specific! So if the question is for example like 'compare A vs B', \
 then you probably want to generate at least two searches, one focussed on A and a seond on B. \
...
may get informed as in 'find features of A for comparison with another entity' )
 - the plan of record should be complete! I.e., all steps should be iuncluded that are believed \
...
guided by the question you need to answer, the answers you have so far, and the plan of record.>",
    "next_step": {{"tool": "<SEARCH or KNOWLEDGE_GRAPH or CLOSER>",
                  "question": "<the question you want to pose to the tool. Note that the \
...
written as a search query.>"}},
    "plan_of_record": "<the updated plan of record, formatted as:
    {{
        "iteration_nr": <the iteration number>,
        "tool": <the tool call you want to make here>,
        "question": <the question you want to have answered in this iteration>
    }}
    >."
 }}
"""
