...
You are given a original SQL statement that returns a list of entities from a table or \
...
Your task will be to identify the source documents that for the entities involved in \
...
Importantly, here are the entity (node) types that you can use, with a short description of what they mean. You may need to \
identify the proper entity type through its description. Also notice the allowed attributes for each entity type and \
their values, if provided. Of particular important is the 'subtype' attribute, if provided, as this is how \
