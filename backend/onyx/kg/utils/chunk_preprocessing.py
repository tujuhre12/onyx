...
from onyx.kg.models import ContextPreparation
...
def prepare_llm_content_extraction(chunk: KGChunkFormat) -> ContextPreparation:
    """
    Calls - prepare the content for the LLM.
    """
    ...
    if chunk.source_type.lower() in [
        call_type.value.lower() for call_type in OnyxCallTypes
    ]:
        llm_context_prompt = CALL_CHUNK_PREPROCESSING_PROMPT
    else:
        llm_context_prompt = GENERAL_CHUNK_PREPROCESSING_PROMPT

    llm_context = llm_context_prompt.format(
