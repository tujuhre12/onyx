  ...
  return (
    <div className="w-full">
      ...
      {notifications && notifications.length > 0 ? (
          ...
          sortedNotifications
            ...
            .map((notification) => {
              ...
              return (
                <div
                  key={notification.id}
                  className="w-72 px-4 py-3 border-b last:border-b-0 hover:bg-gray-background-50 transition duration-150 ease-in-out"
                >
                  <div className="flex items-start">
                    ...
                    <div className="flex-grow">
                      <p className="font-semibold text-sm text-gray-background-800">
                        New Assistant Shared: {persona?.name}
                      </p>
                      {persona?.description && (
                        <p className="text-xs text-gray-background-600 mt-1">
                          {persona.description}
                        </p>
                      )}
                      {persona && (
                        <div className="mt-2">
                          {persona.tools.length > 0 && (
                            <p className="text-xs text-gray-background-500">
                              Tools:{" "}
                              {persona.tools
                                .map((tool) => tool.name)
                                .join(", ")}
                  ...
                  <div className="flex justify-end mt-2 space-x-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1 text-sm font-medium text-gray-background-600 hover:text-gray-background-800 transition duration-150 ease-in-out"
                    >
                      Dismiss
                    </button>
                  </div>
        ...
        <div className="px-4 py-3 text-center text-gray-background-600">
