...
const SidebarSwitch = ({
  ...
  <div className="flex justify-between items-center py-2">
    <span className="text-sm text-gray-background-300">{label}</span>
    <Switch
      checked={checked}
      onCheckedChange={onCheckedChange}
      className="data-[state=checked]:bg-onyx-white data-[state=checked]:border-background-200 data-[state=unchecked]:bg-gray-background-600"
...
const RadioOption = ({
  ...
  <div className="flex items-start space-x-2 mb-2">
    <RadioGroupItem
      value={value}
      id={value}
      className="mt-1 border border-gray-background-600 data-[state=checked]:border-white data-[state=checked]:bg-white"
    />
    <Label htmlFor={value} className="flex flex-col">
      <span className="text-sm text-gray-background-300">{label}</span>
      {description && (
        <span className="text-xs text-gray-background-500">{description}</span>
      )}
    </Label>
  </div>
);
...
export const SettingsPanel = ({
  ...
  return (
    <div
      className="fixed top-0 right-0 w-[360px] h-full bg-background-800 text-gray-background-300 overflow-y-auto z-20 transition-transform duration-300 ease-in-out transform"
      style={{
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            ...
            className="text-gray-background-400 hover:text-white"
          >
            ✕
          </button>
        </div>
