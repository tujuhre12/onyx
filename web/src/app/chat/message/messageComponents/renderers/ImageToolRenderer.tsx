...
export const ImageToolRenderer: MessageRenderer<ImageToolPacket, {}> = ({
  packets,
}: {
  packets: ImageToolPacket[];
}) => {
  const [isExpanded, setIsExpanded] = useState(true);

  const imageStart = packets.find(
    (packet) => packet.obj.type === PacketType.IMAGE_TOOL_START
  )?.obj as ImageToolStart;

  const imageEnd = packets.find(
    (packet) => packet.obj.type === PacketType.IMAGE_TOOL_END
  )?.obj as ImageToolEnd;

  const prompt = imageStart?.prompt;
  const images = imageEnd?.images || [];
