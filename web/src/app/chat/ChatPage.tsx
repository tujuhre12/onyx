...
import { RegenerationRequest, useSendMessage } from "./sendMessage";
...
export function ChatPage({
  ...
  // If chat is turned off, or user not authed, handle that:
  ...
  // The new hook that manages which session is loaded, fetches messages, etc.
  const {
    chatSessionIdRef,
    loadedIdSessionRef,
    selectedChatSession,
  ...
  >(
    existingChatSessionAssistantId !== undefined
      ? availableAssistants.find((a) => a.id === existingChatSessionAssistantId)
  ...
  useEffect(() => {
    // Whenever the session changes & we discover a persona_id, sync with selectedAssistant:
    if (selectedChatSession?.persona_id) {
      const newAsst = availableAssistants.find(
        (asst) => asst.id === selectedChatSession.persona_id
      );
      if (newAsst && newAsst.id !== selectedAssistant?.id) {
        setSelectedAssistant(newAsst);
      }
    }
    // If there's no existing session, we can fall back to default
    else if (!selectedChatSession && defaultAssistantId) {
      const fallback = availableAssistants.find(
        (asst) => asst.id === defaultAssistantId
      );
      if (fallback && fallback.id !== selectedAssistant?.id) {
        setSelectedAssistant(fallback);
      }
    }
  }, [
  ...
  // Basic logic for model overrides:
  const llmOverrideManager = useLlmOverride(
    llmProviders,
    user?.preferences.default_model,
    selectedChatSession // the session object
  );

  // If no assistants are available at all, bounce:
  const allPossibleAssistants = [
    ...availableAssistants,
    ...finalAssistants,
    ...recentAssistants,
  ];
  const anyAssistantAvailable = allPossibleAssistants.length > 0;

  // The actual "live" assistant is either a pinned alternative or the selected one
  ...
  // If the selected (live) assistant also has a default model we want to override:
  useEffect(() => {
  ...
  // Various chat states and logic (some of this might move to a future "useSendMessage" hook, etc.):
  // ----------------------------------------------------------------------------------------------
  ...
  // For uploading files with the current user message:
  const [currentMessageFiles, setCurrentMessageFiles] = useState<
    FileDescriptor[]
  >([]);

  // For displaying documents in the side panel, doc retrieval, etc.:
  const [
    selectedDocuments,
  ...
  // Setting up additional filters for searching. For brevity we keep your existing flow:
  const filterManager = useFilters();
  const availableSources = Array.from(new Set(ccPairs.map((x) => x.source)));
  ...
  useEffect(() => {
    if (liveAssistant) {
      const hasSearchTool = !!liveAssistant.tools.find(
  ...
  // Some state for modals and UI toggles:
  ...
  // Possibly you have your "seed chat from Slack" logic
  const slackChatId = searchParams.get("slackChatId");
  useEffect(() => {
    async function handleSlackChatRedirect() {
      if (!slackChatId) return;
      // You can show a loading state if you want:
      setIsFetchingChatMessages(true);
      try {
        const response = await fetch("/api/chat/seed-chat-session-from-slack", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ chat_session_id: slackChatId }),
        });
        if (!response.ok) {
        ...
        console.error("Error seeding chat from Slack:", err);
        setPopup({ message: "Failed to load chat from Slack", type: "error" });
      } finally {
  ...
  // Cleanup if user is anonymous (force sidebar closed):
  useEffect(() => {
    if (user?.is_anonymous_user) {
      Cookies.set(SIDEBAR_TOGGLED_COOKIE_NAME, "false");
      toggle(false);
    }
  }, [user]);

  // If `?submit-on-load` or `?seeded` logic.  We replicate your original approach:
  const submitOnLoadPerformed = useRef<boolean>(false);
  useEffect(() => {
    // If newly loaded session is brand new (null) AND we have a param to auto-send:
  ...
  // Similarly, if we discover "seeded" and exactly 1 message => onSubmit() again:
  useEffect(() => {
    if (!isFetchingChatMessages && !submitOnLoadPerformed.current) {
      const seededParam =
        searchParams.get(SEARCH_PARAM_NAMES.SEEDED) === "true";
      if (seededParam && messageHistory.length === 1) {
        // Fire once:
        submitOnLoadPerformed.current = true;
        const seededMessage = messageHistory[0].message;
        onSubmit({ isSeededChat: true, messageOverride: seededMessage }).then(
          async () => {
            // If no description, rename:
            if (selectedChatSession) {
              await nameChatSession(selectedChatSession.id);
              refreshChatSessions();
            }
          }
        );
      }
      // Also replicate the "if newMessageHistory.length=2 and session lacks description => rename"
      if (messageHistory.length === 2 && selectedChatSession) {
        nameChatSession(selectedChatSession.id).then(refreshChatSessions);
      }
    }
  }, [isFetchingChatMessages, messageHistory, selectedChatSession]);

  // If user toggles the sidebar open/close:
  ...
  const toggleSidebar = () => {
    if (user?.is_anonymous_user) return;
    Cookies.set(SIDEBAR_TOGGLED_COOKIE_NAME, String(!toggledSidebar));
    toggle();
  };
  const removeToggle = () => {
    setShowHistorySidebar(false);
  ...
  async function onFeedback(
    ...
    if (response.ok) {
      ...
      setPopup({
        message: `Failed to submit feedback - ${json.detail || json.message}`,
        type: "error",
      });
    }
  }
  ...
  const { visibleRange, updateVisibleRangeBasedOnScroll } = useVirtualMessages({
    messageHistory,
    bufferCount: 20,
  });

  // // Then in your scroll handler, figure out which message is “most visible”:
  const displayedMessages = messageHistory.slice(
    visibleRange.start,
    visibleRange.end
  );
  ...
  const { aboveHorizon, scrollToBottom, clientHandleScroll } = useChatScrolling(
    {
    ...
    <>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 pointer-events-none">
        <div className="bg-white p-6 rounded-lg shadow-lg">
          ...
          <p className="mt-4 text-gray-700">{chatState}.</p>
        </div>
      </div>
            ...
            >
              <div className="w-full relative">
                <HistorySidebar
                  ...
                  showShareModal={() => setSharingModalVisible(true)}
                          ...
                          >
                            {(messageHistory.length < BUFFER_COUNT
                              ...
                              const i =
                                messageHistory.length < BUFFER_COUNT
                                  ? fauxIndex
                                  : fauxIndex + visibleRange.start;

                              const messageMap = currentMessageMap();
                              const messageReactComponentKey = `${i}`;
...
//     chatSessions,
//     existingChatSessionId,
//     defaultAssistantId: defaultAssistantIdRaw
//       ? parseInt(defaultAssistantIdRaw)
//       : undefined,
//     refreshChatSessions,
//   });
...
//   const currentSessionChatState = chatState;
//   const currentSessionRegenerationState = regenerationState;
